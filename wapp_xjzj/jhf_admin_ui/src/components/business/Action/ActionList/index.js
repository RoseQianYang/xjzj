...
import {
  actionList as api,
  actionClass as classApi,
} from '../../../../const/api';
import { axiosPost } from '../../../../utils/request';
import { connect } from 'react-redux';
import ActionGoods from './ActionGoods/index';
import axios from 'axios';
import ActionDetail from './Detail';

@connect(state => state.loading, null)
export default class ActionList extends PureComponent {
  constructor() {
    super();
    this.state = { ...initState, actionClass: [] };
  }
  ...
  componentDidMount() {
    ...
    axiosPost(api.get, { page: pagination.current }, data => {
      if (data.hasOwnProperty('data')) {
        axios.get(classApi.all).then(res => {
          this.setState({
            actionClass: res.data.data,
  ...
  // 根据条件渲染不同的对话框
  renderModal = () => {
    const { modalTitle, record, actionClass } = this.state;
    switch (modalTitle) {
      case '添加':
        return <Form add={this.add} actionClass={actionClass} />;
      case '编辑':
        return <Form record={record} edit={this.edit} actionClass={actionClass} />;
      case '查看详情':
        return <ActionDetail id={record.id} />;
      case '活动商品管理':
        return <ActionGoods record={record} />;
      default:
        return null;
    }
  };
  ...
  render() {
    ...
    return (
      <Card>
        ...
        <ModalComponent
          // width={modalTitle === '活动商品管理' ? 1200 : null}
          width={1200}
          title={modalTitle}
